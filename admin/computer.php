...
$query = "SELECT * FROM computer_availability";
$result = $conn->query($query);
$computers = [];
while ($row = $result->fetch_assoc()) {
    $computers[$row['computer_id']] = $row;
}
...
document.addEventListener('click', function(event) {
    ...
    if (!sidebar.contains(event.target) && !toggleBtn.contains(event.target)) {
        ...
        const computers = [];
        const phpComputers = <?php echo json_encode($computers);?>;
        for (let i = 1; i <= 16; i++) {
            const computerId = i;
            const computerState = phpComputers[computerId] || {
    ...
    computers.forEach(computer => {
        ...
        div.className = `computer ${computer.occupied ? "occupied" : "available"}`;
    ...
    document.getElementById("studentName").value = computer.studentName;
...
function saveDetails() {
    if (selectedComputer) {
        ...
        if (!isOccupied) {
            ...
            alert("You cannot mark this computer as occupied.");
        }
    }
}
