...
<body>
    ...
    <div class="container">
        <h2>Log Book History</h2>
                    ...
                    <a href="logbook.php" style="margin-left: 47%; font-size: 16px; color: #159603; text-decoration: none;">back to logbook</a>
...
function displayTable(page) {
    ...
    currentEntries.forEach(student => {
        ...
        const deleteButton = `<button class="delete-btn" onclick="deleteRecord('${student.student_id}')">Delete</button>`;
        ...
        tableBody.innerHTML += `
            <tr>
                ...
                <td>${timeOutButton} ${deleteButton}</td> <!-- Time Out and Delete buttons -->
            </tr>`;
    });
...
document.addEventListener("DOMContentLoaded", function () {
    fetchLogEntries(); // Fetch existing logs when the page loads
    ...
    fetch("php/fetch_log_history.php") // Call new history fetch
        .then(response => response.json())
        .then(data => {
            if (data.error) {
                alert(data.error);
...
function deleteRecord(studentID) {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert(data.error || "Failed to delete record. Please try again.");
        }
    })
